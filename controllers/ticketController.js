...
const verifyTicketUserId = async (req, res) => {
  ...
  try {
    ...
    // Utilisez Promise.all pour exécuter toutes les requêtes en parallèle
    const userPromises = tickets.map(ticket => User.findByPk(ticket.user_id));

    // Attendez que toutes les promesses soient résolues
    const users = await Promise.all(userPromises);

    // Vérifiez si au moins un utilisateur a été trouvé
    ...
    const batchesPromises = tickets.map(ticket => Batch.findByPk(ticket.batch_id));
    const batches = await Promise.all(batchesPromises);
    
    // Vérifiez si au moins un lot a été trouvé
